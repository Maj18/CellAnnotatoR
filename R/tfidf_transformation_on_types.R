...
tfIdfOnType <- function(p2, ann.by.level, clf.data=NULL, topgene=10){
  ...
  top5ifidf.sum.per.type.by.level <- tfidf.per.type.per.gene %>% lapply(function(n) lapply(n,
                                            function(m) head(sort(m, decreasing = T), topgene)%>%sum))
  return(list(s.pos.v3=s.pos.v3, top5ifidf.sum.per.type.by.level=top5ifidf.sum.per.type.by.level)) # a list of hierarchial levels, each level is a vector of per-cell GE sums.
}
