...
tailorResRange <- function(sub.graph, p.left, reann.left, res.min, res.max, res.switch, out.name, outfile.path,
  ...
  check.singleton <- getLayer1KnnClusters(sub.graph=sub.graph, p.left, res.start=res.max, res.step=1)#Increase res until not singleton
...
getNextLayerKnnClusters <- function(p2, annotation, out.name, outfile.path, min.res.start=0,
  ...
  for (cl in names(ann.by.cluster)){
    ...
    new.res.range <- tailorResRange(sub.graph, p.left, reann.left, min.res.start, max.res.middle, res.switch, out.name,
...
getNextLayersKnnClusters <- function(p2, annotation, out.name, outfile.path, layer.n, layer.no.factor=c(0, 0.4),
  ...
  for (l in 2:layer.n){
    if (length(annotation) > 0){
      ...
      #Add sp change
      sp.thresholds$accuracy <- pmax(sp.thresholds$accuracy-layer.no.factor, 0.6) #new
      sp.thresholds$confusion.rate <- pmin((1/sp.thresholds$accuracy)-1, 2/3) #new
...
######Add layer.no.factor
getLeidenHierarchy <- function(p2, out.name, outfile.path, layer.n=3, layer.no.factor=0.4, res.step.layer1=0.01, min.res=0,
